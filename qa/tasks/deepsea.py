...
class DeepSea(Task):
    ...
    the behavior of just this one task (no effect on subtasks):
        ...
        cluster_network: ceph cluster network, 'netwotk ip/netmask'
        public_network: ceph public netwotk, 'network ip/netmask'
    ...
    def _maybe_apply_netwrok(self):
        ...
        if self.cluster_network or self.public_network:
            self.master_remote.run(
                args='sudo mkdir -p %s && ' % network_cfg_dir +
                'sudo touch %s && ' % network_cfg +
                'sudo chown salt.salt %s -R' % network_cfg_dir)
